...
class Auth {
    ...
    protected $config = array(
        ...
        'user_group'        => 'user_group',        // 用户组数据表名
        'auth_group_access' => 'auth_group_access', // 用户-用户组关系表
        'user_group_access' => 'user_group_access', // 用户-用户组关系表
...
//            }
    ...
    public function getUserGroups($uid) {
        ...
        $user_groups = Db::view($this->config['user_group_access'], 'uid,group_id')->view($this->config['user_group'], 'title,rules', "{$this->config['user_group_access']}.group_id={$this->config['user_group']}.id")
            ->where(['uid' => $uid, 'status' => 1])->select();
        $groups[$uid] = $user_groups ? $user_groups : [];
        return $groups[$uid];
    }
