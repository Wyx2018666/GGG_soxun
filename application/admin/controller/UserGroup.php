...
{
    public function lst(){
        $authGroupRes=UserGroupModel::paginate(6);
    ...
    public function edit(){
        if(request()->isPost()){
            ...
            $save=db('user_group')->update($data);
            if($save!==false){
        ...
        $authgroups=db('user_group')->find(input('id'));
    ...
    public function del(){
        $del=db('user_group')->delete(input('id'));
        if($del){
